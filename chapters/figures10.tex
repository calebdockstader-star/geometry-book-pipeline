...
\begin{tikzpicture}[scale=0.868]
  ...
  \draw[aux] (D) -- (E);
  % E is lettered ABOVE its vertex in the book, but the wedge at E is bounded
  % by EA (rising left at 151 deg) and EF (rising right at 15 deg), and a
  % 9pt letter box set at the anchor's 2.2pt clearance lands on EA: the
  % collision audit read "E" as touching it at 0.00pt.  Step the letter out
  % along the wedge's bisector far enough that the whole box clears both
  % sides -- 2.4pt off EA, 4.3pt off EF -- which is where the book has it.
  \foreach \p/\n/\d in {A/A/above, B/B/left, C/C/below left, D/D/below,
                        F/F/above right, G/G/below right}
    {\node[vlab,\d] at (\p) {$\n$};}
  \node[vlab] at ($(E)+(0.10,0.42)$) {$E$};
\end{tikzpicture}
\figcap{10--6}
\end{minipage}\hfill
...
% draws it to that scale, with C directly over B and the break symbol standing
% for the 1-mile gap from B to C.  One tikz unit is one foot.
%
% The break is TWO irregular freehand zigzag lines, not the single mechanical
% sawtooth drawn here before (Caleb: "it reads as a spring").  Both polylines
% are traced off the book's own drawing -- photo PDF p.200 at 220 dpi (book
% p.186), cross-checked against the printed page -- and rescaled onto the
% foot grid.  Note that the band between the two zigzags is CUT OUT of both
% l and m, exactly as the book cuts it: that is what makes the symbol read as
% a break rather than as a drawn object standing between the lines.
% Line ends also follow the book: A sits close to the left end of m, D close
% to the right end of l, and m runs on well past the point where l stops.
\newcommand{\FIGXTEN}{\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=0.62]
  \coordinate (A) at (0.6,0);  \coordinate (B) at (5.6,0);
  \coordinate (C) at (5.6,3);  \coordinate (D) at (10.6,3);
  % l and m, each cut where the break band crosses it
  \draw[fig] (0,3) -- (4.67,3);   \draw[fig] (5.19,3) -- (11.3,3);
  \draw[fig] (0,0) -- (6.25,0);   \draw[fig] (6.70,0) -- (13.2,0);
  \node[vlab,right] at (11.3,3) {$l$};
  \node[vlab,right] at (13.2,0) {$m$};
  % the two hand-drawn break lines, l down to m
  \draw[fig] (4.67,3) -- (4.74,2.11) -- (4.82,1.49) -- (5.26,1.41)
          -- (5.51,1.74) -- (5.77,1.35) -- (6.02,1.33) -- (6.15,1.02)
          -- (6.25,0);
  \draw[fig] (5.19,3) -- (5.28,2.01) -- (5.59,2.25) -- (5.74,2.03)
          -- (5.95,2.08) -- (6.09,1.98) -- (6.33,1.46) -- (6.51,1.49)
          -- (6.55,1.60) -- (6.61,1.51) -- (6.70,0);
  \foreach \p in {A,B,C,D} {\dt{\p}}
  % Each letter stands over (or under) its own 3.2pt dot, so the global 2.2pt
  % clearance leaves the label box just touching the dot.  3.8pt on these four
  % only -- the book likewise sets the letters clear of the dots, not on them.
  \node[vlab,above,outer sep=3.8pt] at (C) {$C$};
  \node[vlab,above,outer sep=3.8pt] at (D) {$D$};
  \node[vlab,below,outer sep=3.8pt] at (A) {$A$};
  \node[vlab,below,outer sep=3.8pt] at (B) {$B$};
\end{tikzpicture}
\figcap{10--10}
\end{bkfigure}}

% ---------------------------------------------------------------- Fig 10-11
\newcommand{\FIGXELEVEN}{\begin{bkfigure}[\linewidth]
...
% ---------------------------------------------------------------- Fig 10-12
% Split out of the old \FIGXTWELVETHIRTEEN: 10-12 belongs to Ex. Group 10-5
% no. 18 and 10-13 to no. 20, so they sit inline after different exercises.
\newcommand{\FIGXTWELVE}{\begin{bkfigure}[\linewidth]
...
% ---------------------------------------------------------------- Fig 10-13
\newcommand{\FIGXTHIRTEEN}{\begin{bkfigure}[\linewidth]
...
% ---------------------------------------------------------------- Fig 10-16
% Split out of the old \FIGXSIXTEENSEVENTEEN: 10-16 belongs to Ex. Group 10-5
% no. 35 and 10-17 to no. 38.
\newcommand{\FIGXSIXTEEN}{\begin{bkfigure}[\linewidth]
...
% ---------------------------------------------------------------- Fig 10-17
\newcommand{\FIGXSEVENTEEN}{\begin{bkfigure}[\linewidth]
...
\begin{tikzpicture}[scale=1.495]
  ...
  \draw[aux] (C) -- (Cp);
  % Right-angle marks at G and at C', as the book has them.  Both are struck
  % at 0.22 of the inner square's side (it was 0.3667), because at the old
  % size the two marks -- they sit at the two ENDS of the same short side
  % GC' -- covered 73% of it between them and merged into an unreadable
  % double box.  Caleb: "the two right-angle squares near G/C collide ...
  % this is the most visible defect."  The book strikes them small and leaves
  % better than half the side clear between them; 0.22 leaves 56%.
  \draw[rmark] (1.1856,1.6392) -- (1.2648,1.5336) -- (1.1592,1.4544);
  \draw[rmark] (1.4544,1.8408) -- (1.5336,1.7352) -- (1.6392,1.8144);
  ...
  \foreach \p/\n/\d in {B/B/below left, A/A/below right, D/D/above left,
    ...
    {\node[vlab,\d] at (\p) {$\n$};}
  % G stepped further up the wedge between the two dashed diagonals: at
  % (1.38,2.10) the letter grazed EG (Caleb: "the G label sits on top of the
  % dashed diagonal").  At (1.30,2.18) it clears EG by 0.36 units (15.5pt at
  % this scale) and DF by 0.55 units, still in the quadrant the book sets it.
  \node[vlab] at (1.30,2.18) {$G$};
\end{tikzpicture}
\figcap{10--20}
\end{bkfigure}}
...
% ---------------------------------------------------------------- Fig 10-22
% Split out of the old \FIGXTWENTYTWOTHREE: 10-22 belongs to Ex. Group 10-7
% no. 1 and 10-23 to no. 10.
\newcommand{\FIGXTWENTYTWO}{\begin{bkfigure}[\linewidth]
...
% ---------------------------------------------------------------- Fig 10-23
\newcommand{\FIGXTWENTYTHREE}{\begin{bkfigure}[\linewidth]
