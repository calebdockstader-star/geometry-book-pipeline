...
% Fig. 15-5 is set INSIDE this group in the book (p.256, left column, directly
% under exercise 1, which is the exercise that cites it), so it is inlined with
% \exfig and the multicols wrapper is dropped -- a figure in a 52 mm column
% would print smaller than the book's own.
\exercisegroup{15--6}
\begin{exlist}
...
% Fig. 15-17 is set INSIDE this group in the book (p.264, head of the right
% column, immediately adjacent to exercise *7, the only exercise that cites
% it), so it is inlined with \exfig and the multicols wrapper is dropped.
\exercisegroup{15--12}
\begin{exlist}
