...
% Re-measured off the plate (PDF 269) 2026-08-17: the axis of abscissas is
% drawn very nearly SYMMETRIC about the axis of ordinates (the old drawing ran
% 4.3 back / 3.9 forward, so its right branch was under-drawn); the quadrant-III
% P sits twice as deep below the axis as it had been drawn; and the II-IV
% segment runs a full unit further right than it had.
\newcommand{\FIGXVTHREE}{\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.014]
  \draw[fig] (-4.39,0) -- (4.41,0);
  \draw[fig] (0,2.52) -- (0,-1.76);
  % same quadrant (I): segment meets neither axis
  \coordinate (P1) at (1.43,2.29);  \coordinate (Q1) at (2.92,1.22);
  \draw[key] (P1) -- (Q1);
  \node[vlab,above=1pt] at (P1) {$P$};
  \node[vlab,right=1pt] at (Q1) {$Q$};
  % quadrant II to quadrant III: crosses the axis of abscissas
  \coordinate (Q2) at (-2.04,1.41); \coordinate (P2) at (-3.36,-1.26);
  \draw[key] (Q2) -- (P2);
  \node[vlab,above=1pt] at (Q2) {$Q$};
  \node[vlab,left=1pt]  at (P2) {$P$};
  % quadrant II to quadrant IV: crosses both axes
  \coordinate (P3) at (-1.08,1.05); \coordinate (Q3) at (3.05,-0.84);
  \draw[key] (P3) -- (Q3);
...
\begin{tikzpicture}[scale=1.040]
  ...
  % branch lengths re-measured off the plate (PDF 270) 2026-08-17, in units of
  % the drawn unit point: ordinate 3.86 back / 1.84 forward, abscissa 1.62 back
  % / 2.27 forward.  The abscissa had been drawn nearly symmetric (1.95/2.05).
  \draw[fig,-latex] (3.86*-\ox,3.86*-\oy) -- (1.84*\ox,1.84*\oy);
  \draw[fig,-latex] (1.62*-\ax,1.62*-\ay) -- (2.27*\ax,2.27*\ay);
  \dt{\ox,\oy}
  \dt{\ax,\ay}
  % lifted 1.2pt clear of its own dot -- at 2.2pt outer sep the numeral's box
  % sat exactly on the dot's rim (collision audit)
  \node[slab,above=1.2pt] at (\ox,\oy) {$1$};
  \node[slab,right=1pt] at (\ax,\ay) {$1$};
...
\begin{tikzpicture}[scale=1.040]
  ...
  % O lifted clear of the axis of abscissas: the -1pt shift had pushed its box
  % down onto the axis (collision audit).  Still above left, as the book sets it.
  \node[vlab,above left=0.7pt and 0pt] at (0,0) {$O$};
...
\begin{tikzpicture}[scale=0.44]
  % The book ticks the axis of ordinates from -3 to 4 (three marks below the
  % axis of abscissas, not two -- counted at 400 dpi on PDF 273); the axis of
  % ordinates was too short to carry the third and it was missing.
  \XVaxes{-4.6}{6.4}{-3.5}{5.4}
  \XVxticks{-4}{5}
  \XVyticks{-3}{4}
...
\begin{tikzpicture}[scale=0.55]
  ...
  % Re-measured off the plate (PDF 274) 2026-08-17 in units of x_2: the axis of
  % abscissas sits 0.21 x_2 below the base line (it had been drawn at 0.39, so
  % nearly twice too low) and the axis of ordinates runs 0.35 x_2 below the base
  % (it had run 0.52).
  \draw[fig,-latex] (-0.53,-1.22) -- (4.95,-1.22);
  % dropped below the arrow tip: our coordinate labels run about twice as wide
  % relative to the drawing as the book's do (see ch15-UNCERTAIN.md), so the
  % foot-label line reaches past the arrow and a level x would sit inside it.
  \node[vlab,below right=0pt and -1pt] at (4.95,-1.06) {$x$};
  \draw[fig,-latex] (0,-1.82) -- (0,2.49);
  \node[vlab,right] at (0,2.49) {$y$};
  \draw[fig] (O) -- (F2);
  \draw[fig] (P1) -- (F1);
  \draw[fig] (P2) -- (F2);
  \draw[aux] (O) -- (P2);
  % the book dots BOTH feet as well as the three named points -- five heavy
  % dots in all; the two feet had been left undotted.
  \foreach \p in {O,P1,P2,F1,F2} {\dt{\p}}
  ...
  % dropped 1.6pt so the pair clears the two foot dots now drawn on the base
  \node[slab,below=1.6pt] at (3.3,\bY) {$(x_1,-C/B)\quad(x_2,-C/B)$};
\end{tikzpicture}
\figcap{15--10}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering
\begin{tikzpicture}[scale=0.60]
  ...
  \draw[fig,-latex] (-1.65,0) -- (5.30,0);
  \node[vlab,right] at (5.30,0) {$x$};
  ...
  \draw[fig] (P3) -- (F3);
  \draw[fig] (P1) -- (P3);
  % the book dots all FIVE points, the two feet (x_2,y_1) and (x,y_1) included;
  % the feet had been left undotted.
  \foreach \p in {P1,P2,P3,F2,F3} {\dt{\p}}
  \node[slab,left] at (P1) {$(x_1,y_1)$};
  \node[slab] at ($(P2)+(-0.34,0.48)$) {$(x_2,y_2)$};
  % the book sets (x,y) above and slightly LEFT of its point, not right
  \node[slab] at ($(P3)+(-0.16,0.50)$) {$(x,y)$};
...
\begin{tikzpicture}[scale=0.52]
  ...
  % lifted clear of the axis of abscissas (collision audit); still above left
  \node[slab,above left=0.7pt and 0pt] at (0,0) {$0$};
...
\begin{tikzpicture}[scale=0.62]
  ...
  % the book sets 0 BELOW LEFT of the origin here (checked on scan15 p.23,
  % which is sharper than the plate); it had been set below right.
  ...
  % P_1 is set above LEFT of its dot in the book, not straight above
  \node[vlab,anchor=south east,xshift=2pt,yshift=-1pt] at (P1) {$P_1$};
  ...
  % SOURCE READING (book p.262 = PDF 276, confirmed three times at 400 dpi and
  % on scan15): the plate prints "(x_2, y_1)" at P_2 -- the SAME pair it prints
  % at Q.  That is a misprint in the book: the prose immediately above names Q
  % as "the point Q (x_2, y_1)", and the Pythagorean step that follows needs
  % P_2 to be (x_2, y_2).  With the prose now transcribed the contradiction is
  % visible on the page, so P_2 is corrected here on Caleb's instruction
  % (FIX-SPEC 2026-08-17 sec.4).  Logged in chapters/ch15-UNCERTAIN.md.
  % set level with the dot, on its right, the way the book sets it
  \node[slab,right,yshift=-1pt] at (P2) {$(x_2,y_2)$};
...
\begin{tikzpicture}[scale=0.756]
  ...
  % M had been tucked in with negative shifts, which put its box on the
  % hypotenuse (collision audit).  Set above left of its dot, clear of the line.
  \node[vlab,anchor=south east,xshift=-1.5pt,yshift=1.5pt] at (M) {$M$};
...
\begin{tikzpicture}[scale=0.936]
  ...
  \draw[aux] (M1) -- (M2);
  % the book dots the two named vertices (a,0) and (b,c) as well as the two
  % midpoints -- four heavy dots; the vertices had been left undotted.
  \foreach \p in {M1,M2,B,C} {\dt{\p}}
...
\begin{tikzpicture}[scale=0.66]
  ...
  % The book sets 0 BELOW LEFT of the origin.  It had been exiled above left
  % because the audit used to chord the arcs; the audit now evaluates the
  % Beziers exactly, so the book's own placement is available again -- the only
  % ink near the origin is the axis of abscissas, and 0.7pt clears it.
  \node[slab,below left=0.7pt and 0pt] at (0,0) {$0$};
  % Re-measured off the plate (PDF 278) 2026-08-17: the centre sits only about
  % 0.10 r above the axis of abscissas (it had been drawn at 0.21 r), so the
  % circle straddles that axis almost evenly, as the book's does.
  \coordinate (K) at (1.04,0.18);
  \def\rr{1.85}
  ...
  \draw[fig] (K) -- (X);
  % the book dots (x,y) where the radius meets the circle as well as the centre
  \dt{K}
  \dt{X}
  % now that the centre sits close to the axis of abscissas, a label set level
  % with it straddles that axis; lifted to above right, still on the book's side
  \node[slab,above right=-1pt and 1pt] at (K) {$(h,k)$};
