...
\begin{tikzpicture}[scale=1.030, every node/.style={font=\scriptsize}]
  % --- row 1
  \draw[given] (0,0) -- (2.0,0);
  \dt{0,0}\dt{2.0,0}
  % the book leaves a clear gap between each letter and its dot
  \node[vlab, above, yshift=2pt] at (0,0.02) {$A$};
  \node[vlab, above, yshift=2pt] at (2.0,0.02) {$B$};
  \node[below=5pt] at (1.0,-0.05) {Line segment};

  \draw[given] (3.3,0) -- (5.3,0);
  \dt{3.3,0}
  \node[vlab, above, yshift=2pt] at (3.3,0.02) {$O$};
  ...
  \begin{scope}[yshift=-2.35cm]
    % the book's triangle points LEFT: a vertex at mid height on the left, the
    % apex up and right of it, the third vertex at the bottom right (measured
    % off the printed figure, 310 x 225 px at 400 dpi)
    \draw[given] (0.05,0.40) -- (1.24,1.27) -- (1.80,0) -- cycle;
    \node[below=5pt] at (0.95,-0.05) {Triangle};
...
\newcommand{\FIGITWO}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.019, every node/.style={font=\scriptsize},
                    hid/.style={fig, dashed, dash pattern=on 2pt off 1.6pt}]
  % ---- cube
  % Three edges meet at the hidden back-bottom-left vertex and ALL THREE are
  % dashed in the book: the back-bottom edge, the back-left vertical, and the
  % connector down to the front-bottom-left corner.  The first draft drew the
  % back-bottom edge solid.
  \begin{scope}
    \def\d{0.42}
    \draw[given] (0,0) rectangle (1.25,1.25);
    \draw[hid]   (\d,\d) -- (\d+1.25,\d);
    \draw[given] (\d+1.25,\d) -- (\d+1.25,\d+1.25) -- (\d,\d+1.25);
  ...
  \begin{scope}[xshift=3.15cm]
    ...
    \draw[hid]   (\d,\d) -- (\d+1.95,\d);
    \draw[given] (\d+1.95,\d) -- (\d+1.95,\d+1.05) -- (\d,\d+1.05);
  ...
  \begin{scope}[xshift=3.15cm, yshift=-6.2cm]
    % the book's ring is THIN -- the hole is about 0.63 of the outer radius,
    % where the first draft drew 0.49 and made the tube look far too fat
    \draw[given] (1.05,0.85) circle (0.82);
    \draw[given] (1.12,0.92) circle (0.52);
    % the tube's cross-section straddles the inner rim on the left: near half
    % solid, far half dashed (the first draft drew one wide flat arc right
    % across the hole instead)
    \draw[given] (0.45,0.92) arc (180:360:0.15 and 0.19);
    \draw[hid]   (0.45,0.92) arc (180:0:0.15 and 0.19);
...
%--------------------------------------------------------------- Fig 1-5
% Ray AC bisects angle BAD -- C placed at exactly half the angle of B.
% Split out of the old \FIGIFIVESIX pair (2026-08-17): Ex. 1-3 #8 cites 1-5 and
% #10 cites 1-6, so each now stands alone after its own exercise, at full
% measure (scale x1.45 for the freed width).
\newcommand{\FIGIFIVE}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.599]
  \coordinate (A) at (0,0);
...
%--------------------------------------------------------------- Fig 1-7
% Right triangle, right angle at the lower-left vertex.  Split out of the old
% \FIGISEVENEIGHT pair: Ex. 1-3 #12 cites 1-7, #13 cites 1-8.
\newcommand{\FIGISEVEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.876]
  \coordinate (R) at (0,0);
  \coordinate (T) at (0,1.75);
  \coordinate (S) at (2.15,0);
  \draw[given] (R) -- (T) -- (S) -- cycle;
  \draw[fig] (0,0.17) -- (0.17,0.17) -- (0.17,0);
  % the book sets 90 deg immediately above the right-angle box, not out in the
  % middle of the triangle (measured off the printed figure)
  \node[slab] at (0.45,0.42) {90\textdegree};
\end{tikzpicture}
\figcap{1--7}
\end{bkfigure}}

%--------------------------------------------------------------- Fig 1-8
% Angle AOB is a right angle; C interior.
\newcommand{\FIGIEIGHT}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.876]
  \coordinate (O) at (0,0);
  ...
  \draw[given] (O) -- (C);
  \dt{A}\dt{B}\dt{C}
  \node[vlab, above, yshift=1.5pt] at (A) {$A$};
...
%--------------------------------------------------------------- Fig 1-9
% A, O, C collinear on l (so angle AOB and angle BOC are supplementary).
% Split out of the old \FIGININETEN pair: Ex. 1-3 #14 cites 1-9, #15 cites 1-10.
\newcommand{\FIGININE}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.656]
...
\begin{tikzpicture}[scale=1.656]
  ...
  \dt{A}\dt{O}\dt{B}
  \node[vlab, above, yshift=1.5pt] at (A) {$A$};
  \node[vlab, above, yshift=1.5pt] at (B) {$B$};
  \node[vlab, below] at (O) {$O$};
\end{tikzpicture}
\figcap{1--10}
\end{bkfigure}}
...
%--------------------------------------------------------------- Fig 1-12
% Equilateral triangle by the two-arc construction (Euclid I.1).
% Split out of the old \FIGITWELVETHIRTEEN pair: Ex. 1-4 #2 cites 1-12,
% #3 cites 1-13.
% 2026-08-17: the book strikes ONLY the two compass arcs, drawn lighter than
% the triangle, and lets them cross of their own accord at the apex and run on
% through the far base vertex.  The first draft added a hand-drawn cross at the
% apex and a tick at each base vertex on top of the arcs -- ink the book does
% not print.  Both removed; the arcs now carry the crossings themselves.
\newcommand{\FIGITWELVE}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.627]
  \def\s{2.05}
  \coordinate (A) at (0,0);
  \coordinate (B) at (\s,0);
  \coordinate (C) at (60:\s);          % exact equilateral apex
  \draw[given] (A) -- (B) -- (C) -- cycle;
  \draw[fig, line width=0.55pt] (A) ++(-18:\s) arc (-18:78:\s);
  \draw[fig, line width=0.55pt] (B) ++(198:\s) arc (198:102:\s);
\end{tikzpicture}
\figcap{1--12}
\end{bkfigure}}

%--------------------------------------------------------------- Fig 1-13
% An angle and its copy -- both drawn at exactly 60 degrees.  The compass arc
% is drawn lighter than the two rays, as the book prints it.
\newcommand{\FIGITHIRTEEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.627]
  \foreach \x/\lo/\lb/\la in {0/{O}/{B}/{A}, 2.75/{O\mkern2mu'}/{B\mkern2mu'}/{A\mkern2mu'}} {
    \begin{scope}[xshift=\x cm]
      ...
      \draw[fig, line width=0.55pt] (V) ++(0:0.62) arc (0:60:0.62);
      % the book marks where the arc cuts each ray: a tick on OB, a cross on OA
      \draw[fig, line width=0.55pt] ($(V)+(0.62,-0.10)$) -- ($(V)+(0.62,0.10)$);
      \draw[fig, line width=0.55pt] ($(V)+(60:0.62)+(15:0.11)$)  -- ($(V)+(60:0.62)+(195:0.11)$);
      \draw[fig, line width=0.55pt] ($(V)+(60:0.62)+(105:0.11)$) -- ($(V)+(60:0.62)+(285:0.11)$);
...
%--------------------------------------------------------------- Fig 1-15
% Angle bisected with compass; Z is the exact arc crossing, so VZ bisects.
% Split out of the old \FIGIFIFTEENSIXTEEN pair: Ex. 1-4 #5 cites 1-15,
% #6 cites 1-16 and 1-17.
\newcommand{\FIGIFIFTEEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.857]
  \coordinate (V) at (0,0);
  \draw[given] (V) -- (26:2.15);
  \draw[given] (V) -- (-26:2.15);
  \coordinate (X) at (26:0.72);
  \coordinate (Y) at (-26:0.72);
  % the vertex arc runs PAST both rays, as the book draws it, and is struck
  % lighter than the rays themselves
  \draw[fig, line width=0.55pt] (V) ++(34:0.72) arc (34:-34:0.72);
  ...
  \draw[given] (V) -- (2.15,0);
  \draw[fig, line width=0.55pt] ($(Z)+(-0.09,-0.09)$) -- ($(Z)+(0.09,0.09)$);
  \draw[fig, line width=0.55pt] ($(Z)+(-0.09,0.09)$) -- ($(Z)+(0.09,-0.09)$);
\end{tikzpicture}
\figcap{1--15}
\end{bkfigure}}
...
%--------------------------------------------------------------- Fig 1-16
% Adjacent angles AOB and BOC sharing ray OB.
\newcommand{\FIGISIXTEEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.857]
  \coordinate (O) at (0,0);
...
%--------------------------------------------------------------- Fig 1-17
% Three pairs of angles, none of them adjacent (traced from scan02 p9):
...
% Split out of the old \FIGISEVENTEENEIGHTEEN pair: Ex. 1-4 #6 cites 1-17,
% #7 cites 1-18.
\newcommand{\FIGISEVENTEEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.499]
...
%--------------------------------------------------------------- Fig 1-18
% Copying a segment onto a line at A.
\newcommand{\FIGIEIGHTEEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.578]
...
%--------------------------------------------------------------- Fig 1-19
% Perpendicular bisector of AB; C is the exact midpoint, l is exactly
% perpendicular (vertical through C).  Split out of the old
% \FIGININETEENTWENTY pair: Ex. 1-4 #8 cites 1-19, #9 cites 1-20.
\newcommand{\FIGININETEEN}{%
\begin{bkfigure}[\linewidth]
...
%--------------------------------------------------------------- Fig 1-20
% Perpendicular to l at a point O on l.
\newcommand{\FIGITWENTY}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.45]
  % same enlargement as Fig 1-19, so the pair matches
  \coordinate (O) at (0,0);
...
%--------------------------------------------------------------- Fig 1-21
% Perpendicular to l from an external point P (m is exactly vertical).
% Split out of the old \FIGITWENTYONETWENTYTWO pair: Ex. 1-4 #10 cites 1-21,
% #11 cites 1-22.
\newcommand{\FIGITWENTYONE}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.672]
  ...
  \draw[fig, line width=0.55pt] (P) ++(222:1.75) arc (222:318:1.75);
  \draw[fig, line width=0.55pt] (-0.09,1.30-0.09) -- (0.09,1.30+0.09);
  \draw[fig, line width=0.55pt] (-0.09,1.30+0.09) -- (0.09,1.30-0.09);
  % arcs of radius 1.45 about those points meet at y = -sqrt(1.45^2 - 1.171538^2)
  \draw[fig, line width=0.55pt] (-0.09,-0.854400-0.09) -- (0.09,-0.854400+0.09);
  \draw[fig, line width=0.55pt] (-0.09,-0.854400+0.09) -- (0.09,-0.854400-0.09);
  % P sits on the compass cross, so its letter is pushed clear of the strokes
  \node[vlab, left, xshift=-3pt] at (P) {$P$};
...
%--------------------------------------------------------------- Fig 1-22
% The three medians; O is the exact centroid.
\newcommand{\FIGITWENTYTWO}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.672]
...
\newcommand{\FIGITWENTYTHREE}{\begin{bkfigure}[0.52\linewidth]
\includegraphics[width=0.52\linewidth]{fig-1-23}%
\figcap{1--23}\end{bkfigure}}
...
%--------------------------------------------------------------- Fig 1-26
% l parallel to m (same direction vector), transversal n; angles 1 and 2 are
% alternate interior angles, hence equal.  Split out of the old
% \FIGITWENTYSIXTWENTYSEVEN pair (Ex. 1-4 #15 cites both, but the book prints
% them as two separate captioned units).
\newcommand{\FIGITWENTYSIX}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.644]
...
%--------------------------------------------------------------- Fig 1-27
% Copying angle 2 at P produces a line through P parallel to l.
\newcommand{\FIGITWENTYSEVEN}{%
\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.644]
