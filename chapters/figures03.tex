...
\begin{tikzpicture}[scale=0.62]
  ...
  \node[vlab,above,yshift=1.5pt] at (P) {$P$};
\end{tikzpicture}
\figcap{3--3}
\end{minipage}\hfill
\begin{minipage}{0.30\linewidth}\centering
...
\begin{tikzpicture}[scale=0.714]
  ...
  % B sits where two drawn lines cross and both run on past it, so its letter
  % goes into the free wedge below the crossing rather than on the tails
  \node[vlab,above left] at (A) {$A$}; \node[vlab] at ($(B)+(265:0.52)$) {$B$};
...
\begin{tikzpicture}[scale=0.806]
  ...
  \node[vlab,below,yshift=-1.5pt] at (C) {$C$};
  \node[vlab,above,yshift=1.5pt] at (D) {$D$};
  \node[vlab,below,yshift=-1.5pt] at (E) {$E$};
...
\begin{tikzpicture}[scale=0.762]
  ...
  \node[vlab,above,yshift=1.5pt] at (A) {$A$};
  \node[vlab,above,yshift=1.5pt] at (B) {$B$};
  \node[vlab,above,yshift=1.5pt] at (C) {$C$};
\end{tikzpicture}
\figcap{3--11}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}\centering
...
\begin{tikzpicture}[scale=0.741]
  ...
  \dt{P} \dt{Q}
  \node[vlab,above,yshift=1.5pt] at (P) {$P$};
  \node[vlab,above,yshift=1.5pt] at (Q) {$Q$};
...
\begin{tikzpicture}[scale=0.741]
  ...
  \node[vlab,above,yshift=1.5pt] at (P) {$P$};
  \node[vlab,below,yshift=-1.5pt] at (Q) {$Q$};
...
\newcommand{\FIGIIIFIFTEEN}{\begin{bkfigure}[0.62\linewidth]
\includegraphics[width=0.62\linewidth]{fig-3-15}%
\figcap{3--15}\end{bkfigure}}
...
% =============================================================== Fig 3-19
% The two spans over the line are proper CURLY BRACES in the book -- each end
% curls down towards l and the middle carries a point up towards its caption.
% The first draft drew them as squared-off brackets.  (Local helper; the name
% is chapter-scoped so it cannot clash with another chapter's figure file.)
% #1 left end, #2 middle, #3 right end, #4 baseline y.
\newcommand{\IIIbrace}[4]{%
  \draw[fig,line width=0.5pt]
    (#1,#4) .. controls (#1,#4+0.10) and (#1+0.07,#4+0.15) .. (#1+0.16,#4+0.15)
    -- (#2-0.16,#4+0.15)
    .. controls (#2-0.07,#4+0.15) and (#2,#4+0.20) .. (#2,#4+0.30)
    .. controls (#2,#4+0.20) and (#2+0.07,#4+0.15) .. (#2+0.16,#4+0.15)
    -- (#3-0.16,#4+0.15)
    .. controls (#3-0.07,#4+0.15) and (#3,#4+0.10) .. (#3,#4);}
...
\begin{tikzpicture}[scale=0.806]
  ...
  \IIIbrace{0.35}{1.40}{2.45}{0.30}
  \IIIbrace{2.65}{3.80}{4.95}{0.30}
  \node[slab,above] at (1.40,0.60) {One side};
  \node[slab,above] at (3.80,0.60) {Other side};
\end{tikzpicture}
\figcap{3--19}
\end{bkfigure}}
...
\begin{tikzpicture}[scale=0.755]
  ...
  \dt{O} \dt{P}
  \node[vlab,above,yshift=1.5pt] at (O) {$O$};
  \node[vlab,above,yshift=1.5pt] at (P) {$P$};
...
\begin{tikzpicture}[scale=0.780]
  ...
  % pushed a touch further left so the letter clears the down-left ray without
  % drifting up towards the O of drawing (i)
  \dt{O2} \node[vlab,left,xshift=-2pt] at (O2) {$O$};
  % (iii) angle AOB, right
  ...
  \node[vlab,above,yshift=1.5pt] at (A3) {$A$};
  \node[vlab,below,yshift=-1.5pt] at (B3) {$B$};
  % (iv) straight angle
  \draw[fig] (0.85,0.00) -- (3.35,0.00);
  \coordinate (O4) at (2.05,0.00); \dt{O4}
  \node[vlab,below,yshift=-1.5pt] at (O4) {$O$};
\end{tikzpicture}
\figcap{3--24}
\end{minipage}\hfill
...
\begin{tikzpicture}[scale=0.806]
  ...
  \node[vlab,above,yshift=1.5pt] at (P) {$P$};
  \node[vlab,above,yshift=1.5pt] at (Q) {$Q$};
  \node[vlab,above,yshift=1.5pt] at (Aa) {$A$};
\end{tikzpicture}
\figcap{3--31}
\end{bkfigure}}
...
\begin{tikzpicture}[scale=0.806]
  ...
  % Both are struck OUTWARD from r and carry an arrowhead where they land on
  % r_1 and on r_2 -- the same "indicating the sides" notation the book sets up
  % in Fig. 3-25(a).  The first draft drew them as two plain arcs.
  \draw[fig,line width=0.5pt,->,>=stealth] ($(O)+(1.06:1.40)$) arc (1.06:28.88:1.40);
  \draw[fig,line width=0.5pt,->,>=stealth] ($(O)+(1.06:1.40)$) arc (1.06:-29.18:1.40);
\end{tikzpicture}
\figcap{3--32}
\end{bkfigure}}
...
\begin{tikzpicture}[scale=0.806]
  ...
  \node[vlab,above,yshift=1.5pt] at (Qa) {$Q_1$};
  \node[vlab,above,yshift=1.5pt] at (Qb) {$Q_2$};
  \node[vlab] at ($(Qc)+(235:0.55)$) {$Q_3$};
  \node[vlab,right] at (Qd) {$Q_4$};
  \node[vlab,above,yshift=1.5pt] at (Qe) {$Q_5$};
\end{tikzpicture}
\figcap{3--33}
\end{bkfigure}}
...
% =============================================================== Fig 3-38
% Split out of the old \FIGIIITHIRTYEIGHTTHIRTYNINE pair (2026-08-17): Ex. 3-6
% *12 cites 3-38 and *13 cites 3-39, so each now stands alone directly after
% its own exercise, at the full measure (scale raised for the freed width).
\newcommand{\FIGIIITHIRTYEIGHT}{\begin{bkfigure}[\linewidth]
...
% =============================================================== Fig 3-39
\newcommand{\FIGIIITHIRTYNINE}{\begin{bkfigure}[\linewidth]
\begin{tikzpicture}[scale=1.15]
  \coordinate (l2L) at (0.30,1.05); \coordinate (l2R) at (4.55,1.25);
  \coordinate (ka) at (1.65,0.15);  \coordinate (kb) at (3.35,2.85);
  \coordinate (O) at (intersection of l2L--l2R and ka--kb);
  ...
  \foreach \p in {P,Q,R} {\dt{\p}}
  \node[vlab,right] at (P) {$P$};
  \node[vlab,above,yshift=1.5pt] at (Q) {$Q_2$};
